...
<body>
	<div class="tab-content">
	    <div class="tab-pane fade in active">
	        <div class="col-md-12">
	             <div class="search">
					<form id="find-page-orderby-form"
						action="${BasePath !}/payableBillLog/list.do" method="post">
			...
			<div id="bill_log_list" class="ff_DataTable"></div>
	    </div>               
	</div>
	...
	$(function(){
		...
		requirejs(['ff/init_datatable'], function(initDataTable){
			dt_purchaseOrder_list = new initDataTable({
				div_id: 'bill_log_list',
				url: rootPath + "/payableBillLog/listAjax.do?billNo=${(bill.billNo) !}",
				columns:[
					...
					{ data: "payableAmountBefore", label: '原应付金额(元)' },
					{ data: "payableAmountAfter", label: '修改后应付金额(元)' },
					{ data: "supplyBefore", label: '原供货价' },
					{ data: "supplyAfter", label: '修改后供货价' },
					{ data: "pointsBefore", label: '原扣点' },
					{ data: "pointsAfter", label: '修改后扣点' }
				],
				//是否显示复选框列，默认：true 显示
	            show_checkbox: false,
	          	//是否显示操作列，默认：true 显示
