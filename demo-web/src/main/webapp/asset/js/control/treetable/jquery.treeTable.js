(function ($) {
    ...
    $.fn.treeTable = function (opts) {
		...
		if (!$treeTable.hasClass('tree_table')) {
        ...
        var pMap = {}, cMap = {};
        var $trs = $treeTable.css({visibility: 'hidden'}).find('tr');
		var trsLen = $trs.length; //SSUI
		var trsObj = {}; // SSUI        

		// SSUI
		$.each($trs, function(idx, ele){

			var id = (typeof $(ele).attr('id') == 'undefined') ? '' : $(ele).attr('id');
			trsObj[id] = ele;
        ...
        function initRelation($trs, hideLevel) {
			...
			var reorderTr = function(parentId){

				setTimeout(function(){
					
					if (parentId in pMap) {
						
						$.each([].concat(pMap[parentId]).reverse(), function(idx, ele){
							
							if (parentId == 0) {
								$treeTable.prepend(trsObj[ele]);
							} else {
								$('#' + parentId).after(trsObj[ele]);
							}
						...
						$treeTable.css({visibility:'visible'});
					}
					
					if (trsLen == 0 && isReordered == false) {
						fatherFindKid();
						isReordered = true;						
					}
            ...
            //标识父节点是否有孩子、是否最后一个节点
            function fatherFindKid(){
				// SSUI: get the reordered trs!!!
				var $tr = $treeTable.css({visibility:'visible'}).find('tr');
