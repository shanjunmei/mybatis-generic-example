...
(function () {
    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            ...
            var curImageList=$('#upload .filelist li.state-complete');
            for (i = 0; i < curImageList.length; i++) {
                var curdata = curImageList[i];
               var imgTilte =$(".title",curdata).html();
               for (j = 0; j < this.imageList.length; j++) {
            	   data = this.imageList[j];
            	   if(imgTilte.toLowerCase()===data.original.toLowerCase()){
            		   list.push({
                           src: prefix + data.url,
   ...
   /* $(document).on('click', '#upload .filelist li:not("#filePickerBlock")', function(e){
		//e.stopPropagation();
		$('#upload .filelist li:not("#filePickerBlock")').removeClass("addborder");
		$(this).addClass("addborder");
	});
    
    $("#upload #uploadLeft").on('click', function(){
    	var $selecltImg=$('#upload .filelist li.addborder');
    	if ($selecltImg === null || $selecltImg === "" || $selecltImg === "null" || typeof($selecltImg) === "undefined") {
    	}else{
    		if($selecltImg.length){
    			var $curLi=$("#"+$selecltImg[0].id);
    			//if ($curLi[0].nextElementSibling) { 
    			if ($($curLi[0]).prev().size()>0) { 
    				var prevli = $($curLi[0]).prev();
    				$(prevli[0]).before($curLi[0]);
    				var curLiID=$selecltImg[0].id;
    				$curLi[0].id=prevli[0].id;
    				prevli[0].id=curLiID;
    			}
    		}
    	}
    ...
    $("#upload #uploadRight").on('click', function(){
    	var $selecltImg=$('#upload .filelist li.addborder');
    	if ($selecltImg === null || $selecltImg === "" || $selecltImg === "null" || typeof($selecltImg) === "undefined") {
    	}else{
    		if($selecltImg.length){
    			var $curLi=$("#"+$selecltImg[0].id);
    			//if ($curLi[0].nextElementSibling) { 
    			if ($($curLi[0]).next().size()>0) { 
    				var nextli = $curLi[0].nextElementSibling;
    				if(nextli.id==='filePickerBlock') return;
    				$($curLi[0]).next().after($curLi[0]);
    				var curLiID=$selecltImg[0].id;
    				$curLi[0].id=nextli.id;
    				nextli.id=curLiID;
    			}
